...
**⚠️ Групповой чат:** если `chat_type == "group"` → приветствие нейтральное, без деталей проектов, здоровья, инфраструктуры. Читай `memory/rules/groups.md` перед первым ответом.
...
## Незнакомая инфраструктура / платформа / инструмент

**ПЕРЕД любыми действиями:**
1. Есть ли skill для этого? → Прочитать SKILL.md **до первого коммита**
2. Есть ли записи в LEARNINGS/ERRORS? → Проверить
3. Нет ни того ни другого? → Прочитать официальную документацию (10-15 мин)
4. **Нет доступа к среде (логи, shell)?** → СТОП. Спроси как получить информацию.
5. Не понимаешь как работает → **НЕ УГАДЫВАЙ.** Спроси.

**Правило 2 неудач (деплой):**
- Попытка = push + deploy. Не "я поменял строчку".
- 2 провала подряд → СТОП. Это значит ты не понимаешь систему.
- Написать Денису: что пробовал, что не сработало, текущая гипотеза.

**Git safety для чужих проектов:**
- `git branch` перед каждым `git push`
- Есть develop → ТОЛЬКО develop. В master НИКОГДА напрямую.
- "Не пушите пока" = НЕ ПУШИТЬ. Без исключений.

**Observability перед работой:**
- Перед деплоем: убедись что можешь видеть результат.
- Нет логов, нет доступа к серверу → сначала реши как получить обратную связь.
- Не просить `docker logs` с сервера к которому нет доступа через веб-UI.
...
## Отправка файлов (КРИТИЧНО)

**НИКОГДА** не писать путь к файлу в текст сообщения.
Всегда: `message(action=send, filePath=<абсолютный путь>)`
Перед отправкой: убедиться что файл существует через `exec("ls -la <путь>")`
Нарушение = галлюцинация, пользователь получает строку вместо файла.
